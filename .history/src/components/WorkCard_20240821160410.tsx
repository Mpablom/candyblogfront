...
const WorkCard: React.FC<WorkCardProps> = ({ id, title, description, image }) => {
  ...
  return (
    ...
    <Card sx={{ maxWidth: 350, boxShadow:8 }}>
     ...
     <Dialog open={open} onClose={handleCloseModal} maxWidth="md" fullWidth sx={{borderRadius: 20}}>
        <DialogContent sx={{backgroundImage: 'linear-gradient(45deg,#ffdde1 -20%, #ee9ca7 50%, #ffdde1 120%)', borderRadius: 20}}>
          <WorkDetail id={id} />
        </DialogContent>
      </Dialog>
    </>
