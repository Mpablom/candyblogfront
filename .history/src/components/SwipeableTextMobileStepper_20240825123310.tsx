...
import SwipeableViews from 'react-swipeable-views/src/SwipeableViews';
...
const SwipeableTextMobileStepper: React.FC<SwipeableTextMobileStepperProps> = ({ gallery, title }) => {
  ...
  return (
    <Box sx={{ maxWidth: 400, flexGrow: 1, margin: 'auto', borderRadius: 2 }}>
      ...
      <MobileStepper
        ...
        nextButton={
          <Button size="small" onClick={handleNext} sx={{fontSize: '1em', color: 'secondary.main'}}>
            Siguiente
            {theme.direction === 'rtl' ? (
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <Button onClick={handleModalBack}>
              <KeyboardArrowLeft sx={{ fontSize: '5em', color: 'secondary.main' }}/>
            </Button>
            {modalImage && (
              <Box
                component="img"
            ...
            <Button onClick={handleModalNext}>
              <KeyboardArrowRight sx={{ fontSize: '5em', color: 'secondary.main' }} />
            </Button>
          </Box>
        </DialogContent>
      </Dialog>
